                ...
                <div class="bg-slate-100 shadow-best rounded-xl mx-4">
                    ...
                    <div class="flex justify-center gap-6 p-4">
                        ...
                        <div
                            ...
                            @if (!empty($detail) && $detail->foto_masuk)
                                <img src="{{ asset('storage/' . $detail->foto_masuk) }}" alt="Foto Masuk"
                                    class="w-28 h-24 rounded-full">
                            @else
                                <img src="{{ asset('img/profile.png') }}" alt="Default Profile"
                                    class="w-32 h-auto rounded-lg">
                            @endif

                            @if (!empty($detail) && $detail->jam_masuk)
                                <div class="mt-2 text-sm font-semibold text-gray-800">
                                    {{ $detail->jam_masuk }} WIB
                                </div>
                            @else
                        ...
                        <div
                            class="flex flex-col items-center border rounded-lg bg-gray-100 shadow-md p-4 w-32 text-center">
                            @if (!empty($detail) && $detail->foto_pulang)
                                <img src="{{ asset('storage/' . $detail->foto_pulang) }}" alt="Foto Masuk"
                                    class="w-28 h-24 rounded-full">
                            @else
                                <img src="{{ asset('img/profile.png') }}" alt="Default Profile"
                                    class="w-32 h-auto rounded-lg">
                            ...
                            @if (!empty($detail) && $detail->jam_pulang)
                                <div class="mt-2 text-sm font-semibold text-gray-800">
                                    {{ $detail->jam_pulang }} WIB
                                </div>
                            @else
                                <div class="mt-2 text-lg font-semibold text-gray-800">
                                    -- : -- : -- WIB
                    ...
                    {{-- form absen --}}
                    @if (!$detail || !$detail->jam_masuk || !$detail->jam_pulang)
                            ...
                            <!-- Tombol Absen Masuk -->
                                <form id="absenMasukForm" action="{{ route('absen.masuk') }}" method="POST"
                                    ...
                                    <input type="hidden" name="pegawai_id"
                                        value="{{ auth('pegawai')->user()->pegawai_id }}">
                    ...
                    @else
                        <div class="flex text-center mt-8">
                            <div class="px-4 text-sm py-2 bg-gray-100 rounded-xl text-gray-600">
                    ...
                    <div class="py-3 text-center">
                        ...
                        <a href="{{ route('absen.form') }}" class="text-blue-700">
                            klik disini
                        </a>
                    </div>
